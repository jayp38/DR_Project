...
from torchvision import models  # added for ResNet50
...
class AptosDataModule(L.LightningDataModule):
    def __init__(self, csv_file, root_dir, batch_size=64, num_workers=4):
        ...
        # augmentations and normalization (images are preprocessed to 256x256)
        self.train_transform = T.Compose([
            T.RandomCrop(224),  # Random crop for augmentation
        ...
        # only crop and normalize for validation
...
class AptosCNN(L.LightningModule):
    def __init__(self, lr=1e-3, num_classes=5):
        ...
        # replace the final classification layer for 5 classes
        num_ftrs = self.model.fc.in_features
        self.model.fc = nn.Linear(num_ftrs, num_classes)
        
        # Freeze earlier layers, fine-tune later layers (e.g., layer4 and fc)
    ...
    def configure_optimizers(self):
        optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.parameters()), lr=self.hparams.lr)  # updated to only optimize trainable params
...
def train_model(epochs=40, batch_size=64, lr=1e-3, resume_from_checkpoint=None):
    ...
    loaded_model = models.resnet50(weights=None)  # Load blank ResNet50 for state_dict compatibility
    num_ftrs = loaded_model.fc.in_features
    loaded_model.fc = nn.Linear(num_ftrs, 5)  # Match the modified fc layer
